// Package bigcachex provides a minimal in-process bigcache-style client.
//
...
// Config is the YAML-driven configuration for bigcachex.
type Config struct {
	// Size is the maximum cache size in bytes.
	Size int `yaml:"size" mapstructure:"size"`
	// TTLSeconds is the default entry TTL in seconds.
	TTLSeconds int `yaml:"ttl_seconds" mapstructure:"ttl_seconds"`
}

// Client is the in-process bigcache stub.
type Client struct {
	cfg Config
}
...
func New(c Config) (*Client, error) {
	if c.Size <= 0 {
		return nil, fmt.Errorf("bigcachex: size must be > 0, got %d", c.Size)
	}
